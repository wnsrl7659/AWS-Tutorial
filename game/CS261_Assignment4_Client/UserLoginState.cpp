...
void UserLoginState::Update()
{
	...
	if (active_task_.is_done())
	{
		try
		{
			...
			std::string const avatar = utility::conversions::to_utf8string(connect_response_data[U("avatar")].as_string());
			configuration_.avatar = avatar;
			//TODO: retrieve the "token" field from the connect response data, and store it in configuration_.token
			std::string const token = utility::conversions::to_utf8string(connect_response_data[U("token")].as_string());
			configuration_.token = token;
			//TODO: retrieve the "game_port" field from the connect response data, and store it in configuration_.game_port
			int const gameport = connect_response_data[U("game_port")].as_integer();
			configuration_.game_port = gameport;
...
void UserLoginState::InitiateUserRequest()
{
	...
	active_task_ = web_client_.request(web::http::methods::POST, U("/api/v1/login"), login_data)
		...
		.then([](web::http::http_response get_users_response) {
			...
			return get_users_response.extract_json();
		});
